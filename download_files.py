...
repos = {
    ...
    'ComfyUI_UltimateSDUpscale': 'https://github.com/ssitu/ComfyUI_UltimateSDUpscale --recursive',
...
def clone_repo_and_install_requirements(repo_url, destination):
    try:
        # Check if the --recursive flag is present in the repo_url
        recursive_clone = '--recursive' in repo_url
        repo_url = repo_url.replace('--recursive', '').strip()
        ...
        if not os.path.exists(full_path):
            if recursive_clone:
                Repo.clone_from(repo_url, full_path, multi_options=['--recursive'])
            else:
                Repo.clone_from(repo_url, full_path)
            print(f"Repository cloned successfully to {full_path}.")
        else:
            print(f"Repository already exists at {full_path}. Skipping clone.")
