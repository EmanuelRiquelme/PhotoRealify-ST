...
def generate_image_by_prompt_and_image(prompt, output_path, input_path, filename, save_previews=False):
  ...
  try:
    ...
    return return_image(prompt_id, server_address, save_previews)
  finally:
    ws.close()
...
def return_image(prompt_id, server_address, allow_preview = False):
  """
  ...
  image = output_images[-1]['image_data']
...
# ---------------------------------------------------------------------------------------------------------------
# Utils
def wait_for_connection(server_address, max_attempts=30, retry_interval=2):
    url = f'http://{server_address}/history/123'
    attempts = 0
    while attempts < max_attempts:
        try:
            ...
            attempts += 1
            print(f"Connection attempt {attempts} failed. Retrying in {retry_interval} seconds...")
            time.sleep(retry_interval)

    print(f"Failed to establish connection to {url} after {max_attempts} attempts.")
    return False
